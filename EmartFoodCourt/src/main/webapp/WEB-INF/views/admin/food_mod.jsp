...
<script>
function submit_chk(){
	var board = document.getElementById("boardform");
	var food_name = board.elements['food_name'].value;
	if(food_name.trim() == ""){	
		alert("음식이름을 작성해주십시오.");
		return false;
	}
				...
				<c:forEach var="total" items="${total}">
					<form action="/EmartFoodCourt/foodModPro.ad" method="post"
						...
						<table>
							<tr>
								<td><input type="hidden" name="connect_num" id="connect_num"
									...
									<td><input type="hidden" name="old_food_name" id="connect_num"
									value="${total.food_name}"></td>
							</tr>
							<tr>
							...
							<tr id="fd_img2">
								<td class="td_left"><label for="food_image">음식 이미지 이름</label></td>
						...
						<section id="commandCell">
							<input type="button" onclick="javascript:submit_chk();" value="등록">&nbsp;&nbsp; <input
								type="reset" value="다시쓰기" /> <a href="javascript:history.back()">이전목록으로</a>
						</section>
						<input type="hidden" name="page" id="page" value="${nowPage}">
						<input type="hidden" name="img_check" id="img_check" value="false">
					</form>
				</c:forEach>
			</c:when>
			<c:otherwise>
